...
namespace AssemblyMethodsVirtualizer.UnitTests
{
  public static class AssemblyDefinitionObjectMother
  {
    ...
    public static AssemblyDefinition CreateMultiModuleAssemblyWithCtorsAndMethodBody ()
    {
      var assembly = CreateMultiModuleAssemblyDefinition();

      MethodDefinition ctor = new MethodDefinition (".ctor", MethodAttributes.Public | MethodAttributes.HideBySig, assembly.MainModule.TypeSystem.Void);
      ...
      ctor.DeclaringType = assembly.MainModule.Types[1];
      assembly.MainModule.Types[1].Methods.Add (ctor);


      MethodDefinition method = new MethodDefinition ("MethodWithNew", MethodAttributes.Public | MethodAttributes.HideBySig, assembly.MainModule.TypeSystem.Void);
      body = method.Body.Instructions;
      body.Add (Instruction.Create (OpCodes.Newobj, ctor));
      body.Add (Instruction.Create (OpCodes.Ret));
      method.DeclaringType = assembly.MainModule.Types[1];
      assembly.MainModule.Types[1].Methods.Add (method);
